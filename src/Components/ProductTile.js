...
const ProductTile = ({products, index, setProducts}) => {
  ...
  const increaseUpVote = () => {
    ...
    Service.post('finalProductVotes', form).then((data) => {
      if (data.success) {
        const updatedProduct = {...products[index]};
        updatedProduct.votes = updatedProduct.votes + 1;
        setProducts(
          products.map((product) =>
            product.uid === products[index].uid ? updatedProduct : product
          )
        );
      }
      /*if (data.code > 200) {
  ...
  const handleUpVote = () => {
    ...
    Service.post('addVote', form).then((data) => {
      if (data.success) {
        increaseUpVote();
      }
      /*if (data.code > 200) {
  ...
  const handleDownVote = () => {
    ...
    form.append('emailId', username);
    form.append('productId', products[index].uid);
    Service.post('removeVote', form).then((data) => {
      if (data.success) {
        decreaseUpVote();
      }
      /*if (data.code > 200) {
    ...
    <div className="child product" style={{marginLeft: '0'}}>
      <div className="product-container">
        ...
        <div className="votes-container">
          ...
          <span>{products[index].votes}</span>
          <span>
            <FontAwesomeIcon
              icon={faChevronDown}
              size="lg"
